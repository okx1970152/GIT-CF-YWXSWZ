  ...
  for (const category of categories) {
    ...
    for (const novel of novels) {
      ...
      if (!entry.hasInfo) {
        ...
        report.errors.push(`[schema] ${category}/${novel} 缺少 info/index.md`);
      } else {
        try {
      ...
      if (entry.chapterCount === 0) {
        ...
        report.summary.invalidNovels += 1;
        report.errors.push(`[files] ${category}/${novel} 缺少 chapters/*.md`);
      }
      if (entry.annotationCount === 0) {
        report.summary.missingAnnotations += 1;
        report.summary.invalidNovels += 1;
        report.errors.push(`[files] ${category}/${novel} 缺少 annotations/*.md`);
      }
...
try {
  ...
  if (report.summary.validNovels === 0 || report.summary.invalidNovels > 0) {
    log("[error] 诊断失败：内容目录存在无效小说，请查看 diagnostics 报告。");
    process.exit(1);
  }
} catch (err) {
